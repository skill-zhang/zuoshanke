...
3. queue: 为每个（合并后/未合并的）任务分配 P1-P4 优先级和依赖链。
   ...
   - deps 为依赖的任务 ID 列表。注意：父节点是子节点的归类，不是子节点的前置条件。子任务完成后父任务自然完成，所以父节点不应设为子的依赖。依赖只在子任务之间有实际先后关系时才设置（如「查出险记录」依赖「拿到车架号」）
   - 不要为已废弃的节点分配队列项
   - 优先级根据对话背景中用户的需求和实际工作流来判断，不是根据节点的树形层级
"""
...
def auto_converge_and_prioritize(db: Session, scene_id: str, tm: ThinkingMap,
    ...
    Args:
        ...
        scene_id: 场景 ID
        tm: ThinkingMap 对象
        summary: 可选，LLM 对当前讨论的摘要说明，用于指导优先级决策

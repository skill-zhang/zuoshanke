...
class MemoryManager:
    ...
    def get_top_for_context(self, query: str = "",
        ...
        # 选取 Top-N，确保 P0 至少有一个位置
        selected = []
        p0_items = [s for s in scored if s[2].priority_level == "P0"]
        other_items = [s for s in scored if s[2].priority_level != "P0"]

        # 先放 P0
        for _, _, m in p0_items[:1]:  # 最多 1 条 P0
            selected.append(m)
            if len(selected) >= max_count:
                break

        # 再放其他
        for _, _, m in other_items:
            if m in selected:
                continue
            selected.append(m)
            if len(selected) >= max_count:
    ...
    def reinforce(self, key: str, boost: float = REINFORCE_BOOST) -> bool:
        ...
        if not mem:
            return False
        mem.explicit_boost = int(max(mem.explicit_boost or 1, 1) * boost)

...
## 深度解剖一下 Hermes 的 Memory 和 Skills 系统——亲身体验后的真实优缺点分析。
## Hermes Memory 系统拆解
### ✅ 优点（要借鉴的）
**① 双存储隔离** — `memory`（Agent笔记）和 `user`（用户档案）分开，职责清晰。用户信息不会和工具配置混在一起。
**② 自动注入** — 每次对话自动加载，agent 不需要手动调用"读取记忆"。这是最关键的体验——AI 应该 **自动记得你**，而不是等你问"你还记得我吗"。
**③ 简单扁平** — 就是个 key-value 存储，上手零门槛。没有复杂的 schema，想记啥记啥。
**④ 多后端可插拔** — SQLite/Mem0/Honcho，能换底层引擎。
### ❌ 缺点（我们要规避的）
**① 全量注入无过滤** — 2200 字符全部塞进 system prompt，**哪怕和当前对话无关**。你花 20 分钟做完一个项目，下一句问"今天天气怎么样"——记忆里还塞着项目细节。纯浪费 token。
**② 无自动提取** — 全靠 Agent 主动调用 `memory()` 来保存。Agent 的判断力不稳定：重要的忘了记，不重要的记了一堆。用户体验割裂。
**③ 无时效衰减** — 一个记忆 3 个月前保存的和 3 分钟前保存的权重一样。用户的偏好会变（搬家了、换工作了、换爱好了），但系统不会自动淘汰旧记忆。
**④ 容量撞墙** — 2200 字符限制到了就报错，不会自动替代最旧/最不重要的条目。用户得手动清理。
**⑤ 无语义检索** — 只能按 key 精确匹配或全量读取。没有"找找关于 XX 项目我学到了什么"这种能力。
## Hermes Skills 系统拆解
### ✅ 优点（要借鉴的）
**① 文件即代码** — SKILL.md 是纯文本 markdown，能 git 管理、能 diff、能 review。比塞在数据库里不知高到哪里去了。
**② 按需加载** — `skill_view()` 只在需要时才加载完整内容，不盲塞到 prompt 里。这个设计对 token 友好。
**③ YAML 元数据** — 有 name/description/tags/version 元信息，能搜索能分类。
**④ 手动可创作** — `skill_manage(action='create')` 让 Agent 在完成任务后自己把经验存成 skill。这是 learning loop 闭环的关键。
### ❌ 缺点（我们要规避的）
**① 全量 skill 描述注入** — 这是 **最大的坑**。93 个 skill 的 name+description 全部塞进 system prompt，就算你只用其中 2 个。我算过大概 3-5K token 的固定开销。Hermes 官方文档自己也承认这是"Biggest token eaters"第三名。
**② 无触发器匹配** — skill 没有 trigger keywords。Agent 只能凭 description 里的文字去"猜"哪个 skill 有用，没有明确的路由信号。
**③ 无自动沉淀** — 跟 Memory 一样，Agent 得主动调用 `skill_manage(action='create')`。现实是 Agent 很少主动触发，导致 skills 全靠用户手动创建。
**④ 扁平无层级** — 93 个 skill 平铺在一个列表里，没有分类树、没有标签体系。找东西靠翻。
**⑤ Hub 技能不可本地修改** — 安装的 hub skill 是只读的，本地想 patch 就得 fork。运维心智负担。
## 坐山客应该怎么做 —— 吸取教训后的优化设计
### 针对 Memory 的优化
痛点	坐山客方案
全量注入浪费 token	**相关性注入** — 只注入与当前对话主题相关的记忆（按 tags/category 匹配）
无自动提取	**会话后自动沉淀** — 每次对话结束后，LLM 扫描本回合内容，自动提取可记忆的 facts 存库
无时效衰减	**带权重的 LRU** — 记忆有 `last_accessed` 字段，长期不用的自动降权，到阈值后归档
容量撞墙	**智能淘汰** — 超限时淘汰权重最低的，而不是硬报错
无语义检索	**关键词标签系统** — 每条记忆有 tags，匹配时按 tags + 关键词双重匹配
### 针对 Skills 的优化
痛点	坐山客方案
全量描述注入	**触发器匹配** — 每个 skill 有 trigger keywords，只注入命中触发器的 skill
无自动沉淀	**对话后提炼** — LLM 在对话中如果能复用某个流程，自动提议创建 skill
扁平管理	**分类目录** — skills 按 category 分目录，API 支持按分类浏览
Hub 不可改	**纯本地无 Hub** — 坐山客的 skills 全部本地创作、本地维护，不存在只读冲突
### 核心设计理念
```
Memory = 用户的事实知识（what）
 Skills = 可复用的流程知识（how）
...
1. 粗筛 [tags + category] → 从全部 memory/skills 中挑出相关候选项
2. 精筛 [关键词匹配]    → 命中 trigger 的 skill + 对话主题匹配的 memory
3. 容量限制 [top-K]     → 最多注入 3 条 memory + 2 个 skill，超限按优先级排序
```
这样既保留了 Mem+Skill 的自动注入优势，又解决了 Hermes 全量注入的 token 浪费问题。

## 这个思路绝了——**把记忆当人来做**，而不是当数据库做。
## 你说得对，Hermes 的 2200 字符限制不是它的设计，是它**没想清楚权重的妥协方案**。硬限制是因为无法取舍，所以一刀切。
## 你的权重模型我展开一下：
## 记忆权重系统（完整设计）
### 权重公式
```
weight = base × recency × frequency × explicit_boost
```
因子	含义	算法
**base**	基础权重	创建时指定，默认 1.0
**recency**	时间衰减	`e^{-λ × days_since_last_access}` — 最近用过的权重高
**frequency**	重复强化	`1 + log₂(times_accessed + 1)` — 反复提及的权重高
**explicit_boost**	显式标记	你说"记住"→ ×2 ，"这个很重要"→ ×3
### 四条等级
等级	权重区间	行为	示例
**P0 🔒**	≥ 8	永不过期，始终注入	姓名"张清泉"、英文名"skill"、居住城市
**P1 ⭐**	4~8	长期保存，高频注入	偏好暖色 UI、常用模型 Qwen3.5
**P2 📝**	2~4	中期保存，按需注入	最近项目"坐山客v0.6天气卡片"
**P3 💤**	< 2	短期保存，逐渐淡出	某次聊到的临时话题
### 自动流转
```
                  新建记忆
                     ↓
      默认 P2（base=2, 初始权重 2）
        ...
        ↓                         ↓
   用户反复提及            超过 30 天未访问
   +explicit_boost         权重持续衰减
        ↓                         ↓
    升 P1 ⭐               降 P3 💤 → 归档
        ↓                         ↓
  超过 90 天未访问        总记忆超限（如 500 条）
  降为 P2 → P3               删除最低权重
```
### Token 预算控制
```
每次只注入 Top-5 条记忆（按 weight 降序）
P0 始终占 1-2 个名额
其余 P1/P2 竞争剩余名额
P3 不在回复开头注入，但 LLM 可以主动查询
```
### "你反复强调"的自动检测
在每次对话的 reply stream 结束后，LLM 自动扫描本回合有没有：
- **重复出现的主题**（同一话题出现 3 次以上）
- **用户纠正**（"不是 A 是 B"）
- **显式标记**（"记住这个"、"这个很重要"、"以后都这样"）
命中任一 → 创建/更新记忆，+1 frequency_count，自动升权。
这其实就是做了 Hermes 没做的三件事：
1. **有优先级** — 不是平铺一堆碎片
2. **会遗忘** — 不重要的自然淡出，不需要手动删
3. **会强化** — 你反复提的东西自动记住，不用每次都喊"记住"

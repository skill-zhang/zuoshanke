...
>
> 设计日期：2026-05-25（初稿），2026-05-26（修订1：漏斗路径；修订2：卡片多入口+产出类型自适应渲染）
> 核心贡献者：张清泉（需求设计）、坐山客（架构设计 & 原型实现）
>
> **核心洞察：工作台卡片 = 场景广场场景的精选子集。卡片是场景的多入口门户——header 进聊天，body 按产出类型自适应渲染直达产出。**
>
> 关联文件：
> - `prototypes/prototype-workbench-v2.0.html`（原型 v2.0 — 多入口卡片 + 产出类型自适应）
> - `prototypes/prototype-workbench-v1.1.html`（原型 v1.1 终版，已存档）
...
>
> **唯一路径：场景 → 场景广场 → 工作台。**
> 不存在绕过广场直通工作台的通道。场景必须先发布到广场（source ≠ draft），再由用户从广场「⭐ 钉到工作台」。
> 对话创建场景也不例外——用户说「加一个 Github Trending 卡片」→ 坐山客建场景 → 发布到广场 → 再钉到工作台。
...
**不存在绕过广场直通工作台的通道。** 所有卡片必经「场景 → 广场 → 工作台」三阶段：

```
场景创建
    │
    ▼
发布到场景广场（source ≠ draft）
    │
    ▼  ⭐ 钉到工作台（用户或坐山客操作）
    │
    ▼
...
坐山客本体 Agent Loop:
  ...
  2. 场景自动发布到广场（source=system）
  3. PATCH /api/scenes/{id} { show_on_workbench: true, workbench_position: 自动排序 }
    │
    ▼
前端工作台自动刷新 → 新卡片出现
```

对话创建时坐山客代用户完成钉操作，但路径仍是完整的三阶段。
...
看到场景卡片 → 点击「⭐ 钉到工作台」
    │                  (或「取消」从工作台移除)
    ▼
前端: PATCH /api/scenes/{id} { show_on_workbench: true }
    │
    ▼
工作台刷新 → 卡片出现（或消失）
```
...
场景广场中每张卡片新增操作按钮：
- 未钉 → 显示「⭐ 加入工作台」
- 已钉 → 显示「⭐ 已加入」（可点击取消，变为未钉态）

**两个入口互为补充：**
- 对话入口：用户直接说「加个XX卡片」，坐山客代劳走完三阶段
- 广场入口：用户逛广场时手动精选，半自助
...
```
┌─ header ─────────────────────────────┐
│ 🌤️ 天气查询              [⭐] [×]    │  ← 点 header → 进入场景聊天
├─ body ───────────────────────────────┤
│                                      │
│   26°C 晴朗 · 北京                   │  ← body 内容按产出类型自适应渲染
│   湿度 45%  风速 3m/s                │     各产出可独立点击 → 直达
│                                      │
├─ footer ─────────────────────────────┤
│            📖 阅读 →                 │  ← 点 footer → 打开产出（默认行为）
└──────────────────────────────────────┘
```

| 区域 | 点击行为 | 说明 |
|------|---------|------|
| **header**（icon + 名称） | 进入该场景的聊天视图 | 所有卡片统一，`handleEnterScene(scene)` |
| **body** | **按产出类型自适应** | 数据展示/缩略图/列表/链接，各有独立交互 |
| **footer**（产出入口） | 打开该场景的产出画廊或主产出 | 兜底入口，有产出时显示 |

body 内的独立点击（`e.stopPropagation()`）不触发整张卡片的 onClick。

### 4.8 产出类型与卡片 body 自适应渲染

场景的产出类型决定卡片 body 如何渲染。判定优先级：**产出文件类型 > 场景 category > 默认**。

| 产出类型 | 判定方式 | body 渲染 | body 内独立点击 |
|----------|---------|----------|---------------|
| **实时数据** | 场景有定时任务产出实时值 | 大号数字 + 多维度标签（天气/股票/汇率） | 无（纯展示） |
| **文本产出** | 产出 .md / .txt 文件 | 最新内容摘要（3行截断） + 「📖 阅读 →」 | 点摘要 → 打开全文 |
| **Web 应用** | 产出 .html 或标记为 webapp | 缩略截图 + 标题 + 「🕹️ 运行 →」 | 点截图/按钮 → 新标签打开 |
| **数据分析** | 产出 .png/.jpg 图表文件 | 缩略图 + 指标数字 + 「📊 查看 →」 | 点图 → 全屏查看 / 导出 |
| **待办清单** | 结构化待办产出（JSON） | 勾选列表 + 完成统计 | 点条目 → 勾选/跳转 |
| **文件集合** | 多个产出文件（无主类型） | 文件类型图标 + 数量统计 + 「📁 浏览 →」 | 点浏览 → 产出画廊 |
| **默认** | 无产出或产出未识别 | 简介文本 + 「进入 →」 | 点 body → 进聊天（同 header） |

**默认卡片示例（无产出）：**

```
┌──────────────────────────────────────┐
│ 📦 Github Trending                [×]│  ← header → 聊天
├──────────────────────────────────────┤
│                                      │
│         坐山客正在准备中…             │  ← body → 进聊天
│         跟我说你想要什么              │
...
**产出丰富时卡片自然变大。** 网格用 `auto-fill, minmax(340px, 1fr)`，高密度产出卡片（如数据分析多张图）在单列时自然撑高，不会截断内容。同一网格行中卡片高度不一（`align-items: start`），视觉上由内容决定重量。
...
| 层 | 文件 | 变更类型 |
|----|------|----------|
| 原型 | `prototypes/prototype-workbench-v2.0.html` | 新增（v2.0 — 多入口卡片 + 产出类型自适应） |
| 原型 | `prototypes/prototype-workbench-v1.1.html` | 存档（v1.1 终版） |
| 原型 | `prototypes/prototype-workbench.html` | 存档（初版） |
| 设计 | `docs/design/schema-v1.3.md` | 本文档 |
| 前端 | `frontend/src/components/WorkbenchView.tsx` | 新增 |
| 前端 | `frontend/src/components/PlazaView.tsx` | 修改（+钉/取消钉 操作按钮） |
...
| 前端 | `frontend/src/index.css` | 修改（+workbench + 广场钉按钮 样式） |
...
| Phase 5 | PlazaView 卡片操作（钉/取消钉） | PlazaView.tsx |
| Phase 6 | 坐山客对话建场景 → 发布广场 → 钉工作台 | 已有流程，只需确保三阶段完整 |
| Phase 7 | 移动端适配 + 动画 | CSS 调整 |
...
| 问题 | 结论 |
|------|------|
| 工作台与场景广场的关系 | **工作台 ⊆ 场景广场**。不存在绕过广场直通工作台的路径 |
| 卡片创建路径 | 场景 → 发布广场 → ⭐ 钉工作台（三阶段漏斗，唯一路径） |
| 广场卡片操作 | 新增「⭐ 加入工作台 / 取消」按钮，与对话入口互为补充 |
| 「进入空间 →」后 Sidebar 状态 | 默认展开「你的伙伴」区域 |
| 工作台对话 session 策略 | 每次用新 session |
| 卡片排序和显隐 | 用户可通过对话调整，也可在广场手动操作 |
| 多用户 | 当前单用户架构，不考虑 |

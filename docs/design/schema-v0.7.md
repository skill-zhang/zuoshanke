# Schema v0.7 — Agent Loop 仪表盘：6 面板收敛可视角 + 收敛决策机制
...
| 收敛（合并相似节点） | LLM 提议 → 用户确认 | LLM 觉得聊透了就主动问「要整理一下吗？」；用户也能一句话直接触发 |
| 排序（优先级队列） | 系统自动（收敛确认后） | LLM 定 P1-P4 + 依赖链 |
| 聚焦执行（Agent Loop） | 系统自动 | 从 PQ 取 P1 自动执行 |
| 纠正 | 用户在聊天框打字 | 唯一用户操作，走对话 |
| 主动收敛 | 用户一句话 | 「收」「走起」「整理一下」「进入执行」— 直接触发收敛 |

**原则**：用户只需要聊天输入框。6 张面板全是信息视图，不需要按钮来触发下一步。但有「🧠思考⏩⚡行动」按钮让用户可以一键收敛。
...
> 核心洞察：**不做「信息是否充分」的判断，而是基于对话节奏的自然信号。**

#### 2.3.1 为什么「充分」不可行

之前的思路是让 LLM 判断「信息是否充分到可以收敛」，但这个条件天然模糊：
- **过早触发？** — LLM 实际有**过早收敛**的倾向（爱给结论、想推进度），而不是过晚
- **永远不触发？** — 只有两种场景会导致 LLM 不收敛：①用户自己很模糊（「帮我看买车的事」），②用户不断提新方向（「顺便看下保险」「贷款呢」）。但这些场景下**本身就不该收敛**——信息确实不充分

结论：设「充分」判断只会引入一个永远剪不断理还乱的模糊地带。

#### 2.3.2 收敛触发条件

**LLM 自主提议（主路径）**：
```
当你觉得对话已经从「了解需求」进入「可以行动」的阶段时，主动问用户要不要收敛。
```
这与 Dialog Engine 的 explore→focus→finalize 阶段转移天然吻合。LLM 自己知道：
- 我正在给用户解释情况 → 探索中
- 我已经在说「那我们来梳理一下」→ 该收敛了

不需要任何硬编码阈值或轮数计数器——LLM 比任何规则都清楚自己聊到哪了。

**用户直接触发（安全阀）**：
| 触发方式 | 例子 | 行为 |
|---------|------|------|
| 一句话 |「收」「走起」「整理一下」「进入执行」| 直接调 converge 工具 |
| 聊天中突然想收 | 用户中途打断说「先整理这些」 | 同样走 converge |
| 🧠思考⏩⚡行动 按钮 | 前端按钮 | 用户行动 |

**用户纠正（非收敛调整）**：
| 例子 | 行为 |
|------|------|
|「不对，先查这个」| LLM 收到纠正，对应调整当前执行方向 |
|「这不是重点，看我说的这个」| LLM 调整聚焦点 |

纠正**不触发全局收敛**，只是在当前执行路径上做方向调整。

#### 2.3.3 收敛决策矩阵

| 场景 | 谁决定触发收敛 | 行为 |
|------|--------------|------|
| LLM 觉得聊透了，主动问「要不要整理一下？」 | LLM 提议 + 用户确认 | LLM 在回复末尾问「要不我整理一下开始干？」用户说「好」→ 执行收敛 |
| 用户自己说「走」「收」「整理一下」 | 用户一言堂 | 跳过提议环节，直接执行收敛 |
| 用户点「🧠思考⏩⚡行动」按钮 | 用户行动 | 同上一言堂 |
| 用户说「不对，先查这个」 | LLM 收到纠正 | 不触发全局收敛，调整当前方向 |
| 用户嗯嗯啊啊不回新信息 | LLM 自然推进 | 连续追问没新信息后，自然会说「那先按这些执行吧」|

#### 2.3.4 为什么不怕「永远不触发」

LLM 有个天然特征——**它喜欢给结论**。实践中更容易发生的是 LLM 聊了 3 轮就提议收敛（用户：还早呢），而不是永远不问。

就算退一万步，用户就是嗯嗯啊啊不回话——LLM 在连续追问没新信息后，也会自然说「那先按这些执行吧」。

而且有「🧠思考⏩⚡行动」按钮和用户一句话触发作为双重安全阀，用户可以随时自己动手。

#### 2.3.5 收敛的执行过程

当收敛被触发（无论是 LLM 提议→用户确认，还是用户直接说「收」）：

1. **LLM 调用 converge 工具**（或内部函数）
2. **执行 `auto_converge_and_prioritize()`** — 读 TM 所有 brainstorm/active 节点
3. **LLM 分析合并** — 合并相似节点、标记废弃、排优先级
4. **写入 PriorityQueue** — P1-P4 + 依赖链
5. **写入 ReflectTimeline** — 记录合并过程
6. **Agent Loop 启动** — 从 PQ 取 P1 开始执行

#### 2.3.6 与 Dialog Engine 的关系

收敛触发是 Dialog Engine 阶段转移的自然结果：

```
EXPLORE（探索需求）
  → LLM 觉得聊透了，问「要不要整理？」
  → 用户确认
  → 自动做 FINALIZE（定稿摘要）+ DECOMPOSE（拆任务）
  → 触发收敛 + 排序
  → EXECUTE（执行）
```

简单的对话题可以在 EXPLORE 阶段内直接推进收敛（LLM 觉得差不多够了）。
复杂的对话题（多轮讨论、多个方向）会在 FINALIZE 阶段触发收敛，此时 TM 和 PQ 已经天然存在。
...
① 发散 → 自动拆解 TM 节点（已有：diverge 端点）
  │
  ├─ [简单问题] → 直接进入收敛提议
  │    LLM 觉得差不多够了 → 问「要不要整理一下？」
  │
  └─ [复杂问题] → LLM 进入探索模式（Dialog Engine）
       │
       ├─ EXPLORE → 多轮对话，追问需求
       ├─ FOCUS → 确认目标范围
       ├─ FINALIZE → 形成方案摘要
...
**位置**：`router/scenes.py` 的 `stream_scene_message()`，在收敛决策通过后调用。

**触发**：收敛决策机制（见 2.3 节）通过后 → LLM 调用 converge 工具（或内部函数）→ 执行此管线。
...
def auto_converge_and_prioritize(db, scene_id, think_map):
    ...
    注意：这是收敛的「执行」步骤。收敛的「决策」由 2.3 节机制控制。
    """
```
...
| LLM 提议收敛的 prompt 表达方式？是直接说「要不要整理？」还是温和地暗示？ | 待实际调优 |
...
- `docs/design/schema-v0.7.md` — 本文（收敛决策机制 + 仪表盘可视化）

 ...
 *
 * 浮在页面顶部中央的未来科技男孩角色。
 * 11 种表情状态，通过护目镜显示不同眼型和嘴巴。
 *
 * Props:
 *   status: AgentStatus  — 当前表情状态
 *   message?: string     — 对话气泡文字（不传则使用默认）
 *   hidden?: boolean     — 是否隐藏角色
 ...
 *     hidden={store.agentHidden}
 *     onToggle={(h) => store.toggleAgentVisibility()}
 *   />
 */
...
interface StateConfig {
  eyes: keyof typeof EYE;
  mouth: keyof typeof MOUTH;
  defaultMsg: string;
  color: string;
  classSuffix: string;
  showLaughTears?: boolean;
  showSadTears?: boolean;
}

const STATE_MAP: Record<AgentStatus, StateConfig> = {
    ...
    <>
      <div className="agent-char-area">
        <div className={`agent-char-container${hidden ? ' agent-char-hidden' : ''} ${animClass}`}>
          <div className="agent-char-bubble">
          ...
          <div className="agent-char-canvas">
            <svg viewBox="10 0 52 50" width="56" height="56" xmlns="http://www.w3.org/2000/svg">
              ...
              {/* Eyes (dynamic) */}
              <g id="visorEyes">
                <path d={eye.L} fill="none" stroke="#00d4ff" strokeWidth="1" strokeLinecap="round"/>
                <path d={eye.R} fill="none" stroke="#00d4ff" strokeWidth="1" strokeLinecap="round"/>
              </g>
              ...
              <g opacity={cfg.showLaughTears ? 1 : 0}>
                ...
                <circle cx="20" cy="12" r="1.2" fill="#00d4ff" opacity="0.4"/>
                <circle cx="52" cy="12" r="1.2" fill="#00d4ff" opacity="0.4"/>
                <circle cx="22" cy="10" r="0.8" fill="#00d4ff" opacity="0.3"/>
                <circle cx="50" cy="10" r="0.8" fill="#00d4ff" opacity="0.3"/>
              </g>
              ...
              <g opacity={cfg.showSadTears ? 1 : 0}>
                ...
                <circle cx="28" cy="32" r="1" fill="#00d4ff" opacity="0.35"/>
                <circle cx="27" cy="38" r="0.8" fill="#00d4ff" opacity="0.25"/>
                ...
                <circle cx="44" cy="32" r="1" fill="#00d4ff" opacity="0.35"/>
                <circle cx="45" cy="38" r="0.8" fill="#00d4ff" opacity="0.25"/>
              </g>
          ...
          {onToggle && (
            <div className="agent-char-hider" onClick={onToggle} title="隐藏/显示角色">
              {hidden ? '👁' : '✕'}
            </div>
          )}
        </div>
